...
describe('set-spec.js', () => {
  it('support set', () => {
    ...
    expect(new XTemplate(tpl).render(data)).toEqual('2-3|4-6|');
  });
  ...
  it('support set on top', () => {
    const tpl = '{{set(n=1)}}{{n}}';
    expect(new XTemplate(tpl).render({})).toEqual('1');
  });

  it('support property refer as key', () => {
    const tpl =
      '{{set(x=[0])}}{{#each(data)}}{{set(x[0] = 1 + x[0])}}{{/each}}{{x[0]}}';
    expect(
  ...
  it('support modify parent scope data', () => {
    const tpl =
      '{{set(d1=1,d2=2)}}{{#with(data)}}{{set(d1=2,../d2=4)}}{{/with}}{{d1}}{{d2}}';
    expect(
      new XTemplate(tpl).render({
        data: {},
      }),
