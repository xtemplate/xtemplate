...
var functionStatementStore = {};

nativeCode = nativeCode.concat([
     'name = tpl.name;',
     'pos = tpl.pos;',
     'runtime = tpl.runtime;',
     'nativeCommands = root.nativeCommands;',
     'utils = root.utils;'
]);
...
function genFunction(self, statements) {
  ...
  for (var i = 0, len = statements.length; i < len; i++) {
    ...
    statementSource = self[statement.type](statement).source;
    pushToArray(statementsSotre, statementSource);
    pushToArray(source, statementSource);
  }
  source.push(RETURN_BUFFER);
  source.push('}');
  // avoid deep closure for performance
  pushToArray(self.functionDeclares, source);
  functionStatementStore[functionName] = statementsSotre;
  return functionName;
}
...
//    source.push('return tryRun(this);');
  ...
  return {
    params: ['undefined'],
    source: source.join('\n'),
    fnSource: self.functionDeclares.join('\n')
  };
}
...
function generateFunction(self, func, block, escape) {
  ...
  if (idString in nativeCommands) {
    ...
    } else if (idString === 'if' && !elseIfsName) {
      var optionIdName = guid(self, 'option');
      source.push("var " + optionIdName + ' = ' + functionConfigCode.exp  + ";");
      source.push('if(' + optionIdName + '.params[0]){');
      source = source.concat(functionStatementStore[fnName]);
      if (inverseName){
        source.push('} else {');
        source = source.concat(functionStatementStore[inverseName]);
        source.push('}');
      } else {
        source.push('}');
      }
...
compiler = {
  ...
  compile: function (tplContent, name, config) {
    ...
    var s = 'var tpl,root,name,pos,Scope,nativeCommands,utils,callFnUtil,callDataFnUtil,callCommandUtil,rangeCommand,voidCommand,foreachCommand,forinCommand,eachCommand,withCommand,ifCommand,setCommand,includeCommand,parseCommand,extendCommand,blockCommand,macroCommand,debuggerCommand;\n'+
        code.fnSource +
        'return  function('+code.params.join(',')+'){\n'+
            code.source + substitute(SOURCE_URL, {name: name})+
        '\n};';
    var fn = Function.apply(null,["", s]);
    return fn();
  }
};
