...
var DIRECT_SCOPE_RESOLVE = 'var {lhs} = scope.data{idParts};';
var IF_DIRECT_SCOPE_RESOLVE = [
    'var {lhs} = directAccess ? scope.data{idParts} : scope.resolve([{idPartsArr}]);'
].join('\n');
var DIRECT_ROOT_SCOPE_RESOLVE = 'var {lhs} = scope.root.data{idParts};';
...
xtplAstToJs = {
    ...
    id: function (idNode, extra) {
        ...
        if (!isComplex) {
            ...
            var remain;
            if (part0 === 'this') {
                remain = idParts.slice(1).join('.');
                if (remain) {
                    remain = '.' + remain;
                }
                source.push(substitute(DIRECT_SCOPE_RESOLVE, {
                    lhs: idName,
                    idParts: remain
                }));
                return {
                ...
                source.push(substitute(DIRECT_ROOT_SCOPE_RESOLVE, {
                    lhs: idName,
                    idParts: remain
                }));
                return {
                ...
                source.push(substitute(IF_DIRECT_SCOPE_RESOLVE, {
                    lhs: idName,
                    idParts: remain,
