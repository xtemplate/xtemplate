...
var IF_AFFIX_DIRECT_SCOPE_RESOLVE_TOP = [
        'var {lhs} = directAccess ? ' + directScopeRead() + ': scope.resolve([{idPartsArr}]);'
].join('\n');
var IF_AFFIX_DIRECT_SCOPE_RESOLVE = [
        'var {lhs} = ' + directScopeRead() + ';'
].join('\n');
var IF_AFFIX_DIRECT_ROOT_SCOPE_RESOLVE = 'var {lhs} = ' + directScopeRead(1) + ';';

var IF_AFFIX_DIRECT_SCOPE_RESOLVE_TOP_NO_FIELD = [
        'var {lhs} = directAccess ? ' + directScopeReadNoField() + ': scope.resolve([{idPartsArr}]);'
].join('\n');
var IF_AFFIX_DIRECT_SCOPE_RESOLVE_NO_FIELD = [
        'var {lhs} = ' + directScopeReadNoField() + ';'
].join('\n');
...
function opExpression(e, extra) {
    var source = [],
        ...
        code1Source,
        code2Source,
        code1 = xtplAstToJs[e.op1.type](e.op1, extra),
        code2 = xtplAstToJs[e.op2.type](e.op2, extra);
    exp1 = code1.exp;
    exp2 = code2.exp;
    var exp = guid('exp');
...
function genTopFunction(xtplAstToJs, statements) {
    ...
    for (var i = 0, len = statements.length; i < len; i++) {
        pushToArray(source, xtplAstToJs[statements[i].type](statements[i], {
            top: 1
        }).source);
    }
    source.push(RETURN_BUFFER);
        ...
        if (!isComplex) {
            ...
            if (part0 === 'this') {
                ...
                source.push(substitute(remain ?
                    ...
                    IF_AFFIX_DIRECT_SCOPE_RESOLVE_NO_FIELD, {
                    lhs: idName,
                    idParts: remain
                }));
                return {
                ...
                source.push(substitute(remain ?
                    ...
                    IF_AFFIX_DIRECT_ROOT_SCOPE_RESOLVE_NO_FIELD, {
                    lhs: idName,
                    idParts: remain
                }));
                return {
                ...
                source.push(substitute(remain ?
                    IF_AFFIX_DIRECT_SCOPE_RESOLVE_TOP :
                    IF_AFFIX_DIRECT_SCOPE_RESOLVE_TOP_NO_FIELD, {
                    lhs: idName,
                    idParts: remain,
